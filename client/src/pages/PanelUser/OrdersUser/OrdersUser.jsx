        ...
        <div className={`${styles.ordersPreparation} flex `}>
            <div className={` flex `}>
             ...
             <div className={`${styles.ordersPendings} `}>
                ...
                <div>
                    {orderHistoryUser.map((order) => (
                        <div key={order._id} className={styles.orderCard}>
                            <p><span className={styles.spanTitle}>Número de órden: </span> {order._id}</p>
                            <h4>
                                <span className={styles.spanTitle}>Dirección de entrega: </span>
                                {order.address && order.address[0] && order.address[0].street}, {order.address && order.address[0] && order.address[0].city}
                            </h4>
                ...
                {/* <div>
                    {userOrderActivesInPreparation.map((order) => (
                        <div key={order._id} className={styles.orderCard}>
                            <p><span className={styles.spanTitle}>Número de órden: </span> {order._id}</p>
                            <h4><span className={styles.spanTitle}>Dirección de entrega: </span>{order.address[0].street}, {order.address[0].city}</h4>
                            <p><span className={styles.spanTitle}>Status: </span> {order.status}</p>
                ...
                <div>
                    {userOrderActivesOnTheWay.map((order) => (
                        <div key={order._id} className={styles.orderCard}>
                            <p><span className={styles.spanTitle}>Número de órden: </span> {order._id}</p>
                            <h4><span className={styles.spanTitle}>Dirección de entrega: </span>{order.address[0].street}, {order.address[0].city}</h4>
                            <p><span className={styles.spanTitle}>Status: </span> {order.status}</p>
