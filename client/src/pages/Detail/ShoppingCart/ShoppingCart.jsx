...
export default function ShoppingCart() {
    ...
    function handleSubmit() {
        if (selectedAddress) {
            ...
            if (addressUser && addressUser.length === 0) {
                navigate(`/panelUser/allInfoUser/postAddress/${idUser}`);
            } else {
            ...
            <div>
                {addressUser?.map((el) => {
                    ...
                    <div key={el._id} className={`${styles.address} jcspaceBetween`}>
                        <input type="radio" name="address" onClick={() => setSelectedAddress(el)} />
                        <div>
    ...
    // };
                                                    ...
                                                    <span className={styles.counter}>{el.quantity}</span>
...
import { useDispatch, useSelector } from "react-redux";
import { useNavigate, Link } from 'react-router-dom';
import { getIdProduct, postOrders } from '../../../redux/actions/index';
import styles from './ShoppingCart.module.css';
...
export default function ShoppingCart() {
    ...
    useEffect(() => {
        ...
        if (order) {
            const parsedOrder = JSON.parse(order);
            setOrderData(parsedOrder);
            setIdProduct(parsedOrder.idProduct);
            dispatch(getIdProduct(parsedOrder.idProduct));
        }
    }, [dispatch]);

    function handleSubmit() {
        dispatch(postOrders(idUser, orderData)).then(() => {
            localStorage.removeItem('order');
            if (!userInfoLogin) {
                setShouldRedirect(true);
            }
