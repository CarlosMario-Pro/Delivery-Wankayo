...
import { getOrdersOnTheWay, changeStatusOrder, changeStatusOrderPrevious } from '../../../../redux/actions/index';
import { useDispatch, useSelector } from 'react-redux';
import PanelAdmin from '../../PanelAdmin';
import Options from '../Options/Options';
import styles from './OrdersOnTheWay.module.css';


export default function OrdersOnTheWay() {
    const dispatch = useDispatch();
    const ordersOnTheWay = useSelector((state) => state.ordersOnTheWay);
  
    //Trae todas las órdenes pendientes (La nuevas órdenes)
    useEffect(() => {
        dispatch(getOrdersOnTheWay());
    }, [dispatch]);
    ...
    const handleConfirmChangeStatus = (idOrder) => {
        dispatch(changeStatusOrder(idOrder, { "status": "Entregado" }));
        setShowChangeStatusModal((prevState) => ({
            ...prevState,
            [idOrder]: false,
        }));
    ...
    const handleChangeStatusOrderPrevious = (idOrder) => {
        dispatch(changeStatusOrderPrevious(idOrder, { "status": "En preparación" }));
    };
        ...
        <div className={`${styles.ordersPreparation} flex `}>
            <div className={` flex `}>
             ...
             <div className={`${styles.ordersPendings} `}>
                <Options />
                <h1>Órdenes En Camino</h1>
                <div>
                    {ordersOnTheWay.map((order) => (
                            ...
                            <div className={`${styles.order} jcspaceBetween`}>
                                ...
                                <div className={`${styles.fff} `}>
                                    <div className={`${styles.buttons} centerColumnSpaceBetween `}>
                                        <button className={`${styles.buttonUpdatePrevius} `} onClick={() => handleChangeStatusOrderPrevious(order._id)}>Devolver a "En Preparación"</button>
                                        <button className={`${styles.buttonUpdate} `} onClick={() => handleShowChangeStatusModal(order._id)}>Cambiar a "Entregado"</button>
                                    </div>
